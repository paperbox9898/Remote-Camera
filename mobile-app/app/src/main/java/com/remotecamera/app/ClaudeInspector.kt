...
class ClaudeInspector(
    ...
    private val apiUrl: String = CLAUDE_API_URL,
) {
    suspend fun inspect(
        jpegBytes: ByteArray,
        ...
        val base64Image = jpegBytes.toByteString().base64()
        ...
        client.newCall(request).execute().use { response ->
            ...
            val parsed = try {
                JSONObject(extractJsonObjectText(rawText))
            } catch (e: Exception) {
            ...
            ClaudeInspectionResult(
                ...
                falsePositiveLikely = parsed.optBoolean(
                    "falsePositiveLikely",
                    parsed.optBoolean("false_positive_likely", false),
                ),
            )
        }
    }
    ...
    private fun buildSafetyPrompt(context: ClaudeInspectionContext): String {
        val areaText = if (context.areaConfigured) {
            ...
            "별도 감시 영역이 없으므로 프레임 전체의 사람 감지를 기준으로 합니다."
        }
        val customPrompt = context.customPrompt.trim().takeIf { it.isNotBlank() }?.let {
            "\n\n사용자 검사 프롬프트:\n$it"
        }.orEmpty()
...
설비, 리프트, 바닥, 차체 아래, 작업자 자세, 가림, 조명 문제를 함께 확인하세요.
...
status 값은 SAFE, WATCH, DANGER 중 하나입니다.
severity 값은 NONE, LOW, MEDIUM, HIGH, CRITICAL 중 하나입니다.
